...
(defun at-cnc::lwpl2gcode (ent / pts bulges route cnc-f i times rub)
  ...
  (while (>= times 0)
	...
	(setq offset-result (vl-catch-all-apply 'vla-offset
		    (list (e2o ent)
    ...
    (if (vl-catch-all-error-p offset-result)
      (princ (strcat "
刀路偏移失败: " (vl-catch-all-error-message offset-result)))
      (progn
    (setq route (entlast))
...
(defun at-cnc::circle2gcode (ent / pts bulges route cnc-f i times rub kthickness u)
  ...
  (if (= 1 (@:get-config '@cnc::syntek))
	...
	(progn
	  ...
	  (exit)))
    (if (= 3 (entity:getdxf ent 62))
	(setq offset-result (vl-catch-all-apply 'vla-offset (list (e2o ent)
		    (* -0.5 (+ (@:get-config '@cnc::r)
			       (* times
				  (/ kthickness
				     (@:get-config '@cnc::k-times))))
